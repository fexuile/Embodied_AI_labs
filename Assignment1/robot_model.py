...
class RobotModel:
    ...
    def fk(self, qpos: np.ndarray) -> np.ndarray:
        ...
        t = np.zeros(3)
        poses = []
        poses.append(np.block([[R, t[:, None]], [0, 0, 0, 1]]))
        pose = np.block([[R, t[:, None]], [0, 0, 0, 1]])
        i = 0
        for joint in self.joints:
            if isinstance(joint, RevoluteJoint):
                theta = qpos[i]
                axis = joint.axis
                rotation = axis_angle_to_mat(axis * theta)
                R = joint.rot @ rotation
                t = joint.trans
                pose = pose @ np.block([[R, t[:, None]], [0, 0, 0, 1]])
                poses.append(pose)
                i += 1
            else :
                R = joint.rot
